...
from utils import read_truths_args, read_truths, get_all_files, get_camera_intrinsic, id2kor
...
class listDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        c_id = imgpath.split('/')[-1].split('_')[0]
        c_name = id2kor(c_id)
        #imgpath = '../test_datasets' +'/'+ c_id + '.' + c_name + '/'+ c_id + '.' + c_name+'.원천데이터/'+ c_id + '.' + c_name +'.TR/' + imgpath.split('/')[-1]
        imgpath = 'new_dataset' +'/'+ c_id + '.' + c_name + '/'+ c_id + '.' + c_name+'.원천데이터/'+ c_id + '.' + c_name +'.TR/' + imgpath.split('/')[-1]
        ...
        if self.train:
            ...
            campath = imgpath.replace('원천', '라벨링').replace('.TR', '.3D_json').replace('_NT','_TR').replace('.jpg', '.json').replace('.png','.json')
            labpath = imgpath.replace('원천', '라벨링').replace('.TR', '.using_data').replace('_NT','_TR').replace('.jpg', '.txt').replace('.png','.txt')
            ...
            camera_info = {}
            with open(campath) as f:
                label_info = json.load(f)
                # Read intrinsic camera parameters
                camera_info['fx'] = float(label_info['metaData']['Fx'])
                camera_info['fy'] = float(label_info['metaData']['Fy'])
                camera_info['u0'] = float(label_info['metaData']['PPx'])
                camera_info['v0'] = float(label_info['metaData']['PPy'])
            return (img, label, camera_info)
