...
# 
def id2eng(id):
    class_dict = {'0203':'bowl',
                '0104':'beverage_bottle',
                '0502':'cresting',
                '0504':'fishbowl',
                '0503':'flower_bottle',
                '0101':'glass_bottle',
                ...
                '0202':'plate',
                '0701':'pouch',
                '0103':'sause_container',
                '0102':'spray_bottle',
                '0201':'storage_bowl',
                '0301':'tumbler',
                '0603':'props_box',
                '0801':'glass_door',
                '0403':'measuring_container',
                '0604':'furniture',
                '0702':'perfume_container',
                '0703':'cosmetic_container',
                '0401':'beaker',
                '0602':'bookshelf',
                '0505':'glass_ornaments',
                '0501':'trophy'}
...
def json2str(lab_path):
    lab = '3 '
    with open(lab_path) as f:
        data = json.load(f)
        location = data['labelingInfo'][0]['3DBox']['location'][0]
        lab += '{} '.format(float(location['x9']) / 1280)
        lab += '{} '.format(float(location['y9']) / 720)
        lab += '{} '.format(float(location['x4']) / 1280)
        lab += '{} '.format(float(location['y4']) / 720)
        lab += '{} '.format(float(location['x1']) / 1280)
        lab += '{} '.format(float(location['y1']) / 720)
        lab += '{} '.format(float(location['x8']) / 1280)
        lab += '{} '.format(float(location['y8']) / 720)
        lab += '{} '.format(float(location['x5']) / 1280)
        lab += '{} '.format(float(location['y5']) / 720)
        lab += '{} '.format(float(location['x3']) / 1280)
        lab += '{} '.format(float(location['y3']) / 720)
        lab += '{} '.format(float(location['x2']) / 1280)
        lab += '{} '.format(float(location['y2']) / 720)
        lab += '{} '.format(float(location['x7']) / 1280)
        lab += '{} '.format(float(location['y7']) / 720)
        lab += '{} '.format(float(location['x6']) / 1280)
        lab += '{} '.format(float(location['y6']) / 720)
        lab += '{} '.format(float(location['x-range']) / 1280)
        lab += '{}'.format(float(location['y-range']) / 720)
    return np.array(list(map(float, lab.split())))

def read_truths(lab_path, num_keypoints=9):
    num_labels = 2*num_keypoints+3 # +2 for width, height, +1 for class label
    if os.path.getsize(lab_path):
        truths = np.loadtxt(lab_path)
        #truths = json2str(lab_path)
...
def compute_iou(name, frame, vis):
    ...
    sz = [720, 1280]
    if vis==1:
        proj = np.loadtxt("data/{}/model/vis/prj_{}.txt".format(name, frame))
        face = np.loadtxt("data/{}/model/vis/ind_{}.txt".format(name, frame))
    elif vis==2:
    ...
    # compute IoU score
    c_id = eng2id(name)
    gmask = cv2.imread("data/{}/masks/{}_00{}_TR_b.png".format(name, c_id, frame),0)
    ...
    if vis==1:
        ...
        cv2.imwrite("data/{}/model/vis/mask_{}.png".format(name, frame), pmask_*255)
    elif vis==2:
        pmask_ = np.uint8(pmask)
